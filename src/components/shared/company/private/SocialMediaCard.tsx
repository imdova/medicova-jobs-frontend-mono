...
const SocialMediaCard: React.FC<SocialMediaCardProps> = ({ company }) => {
  const dispatch = useAppDispatch();
  useCompanyData(company?.id);
  const socialLinks = company?.socialLinks as SocialMediaLinks;
  ...
  const handleUpdate = async (formData: SocialMediaLinks | null) => {
    dispatch(
      updateCompany({ id: company?.id, updates: { socialLinks: formData } }),
    );
    setIsModalOpen(false);
  };
