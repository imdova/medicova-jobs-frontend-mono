...
const SavedJobsPage: React.FC<{ jobs?: JobData[]; total?: number }> = ({
  jobs = [],
  total = 0,
}) => {
  ...
  const { jobs: savedJobs, loading } = useAppSelector(
    (state) => state.savedJobs,
  );

  const filteredJobs = jobs.filter((j) => savedJobs?.includes(j.id));
