...
import { API_MAIL_CHANGE_COMPANY } from "@/api/employer";
...
export default function VerifyCompanyEmailPage() {
  ...
  const redirect = () => {
    if (typeof window !== "undefined") {
      window.location.replace("/employer/setting/company-info");
    }
  };
  ...
  useEffect(() => {
    ...
    const verifyEmail = async () => {
      try {
        const response = await fetch(API_MAIL_CHANGE_COMPANY, {
          method: "POST",
          headers: {
